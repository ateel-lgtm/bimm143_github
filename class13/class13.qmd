...
Today we will perform an RNAseq analysis in thr effects of dexamethasone (hereafter "dex"), a common steroid, on airway smooth muscle (ASM) cell lines.
...
We have 4 replicates per condition ("control" and "treated"). We want to compare the control vs the treated to see which gene expression levels change when we have the drug present.

We will be splitting the 4 groups up and then compare the rows of each of the 4 groups. Going row by row will be going gene by gene to see if the values of control columns is different than the values in treated columns. 
...
If we consider "treated/control' counts we will get a numner that tells us the change
...
>Q. Add a new column `log2fc` for fold change of treated/control to our `meancounts` object.
...
The main function in this package is called `DESeq()` it will run the full analysis for us on out `dds` input object: 
...
> Q. Use ggplot2 to make a first version "Volcano plot" of `log2foldchange` vs `padj`
...
We will use 2 bioconductor packages for this "mapping" **AnnotationDbi** amd **org.Hs.eg.db**.
...
Now we know the gene names (gene symbols) and their entrez IDs we can find out pathway they are involved in. This is called "pathway analysis" or "gene set enrichment"
...
To run our pathway analysis we will use the **gage()** function. It wants two main inputs: a vector of importance (in or case the log2 fold change values); and the gene sets to check overlap for. 
