...
Today we will perform an RNAseq analysis in thr effects of dexamethasone
...
We need to check that the metadata matches the sample in our count data.
...
compare the control vs the treated to see which gene expression levels
change when we have the drug present.

We will be splitting the 4 groups up and then compare the rows of each
of the 4 groups. Going row by row will be going gene by gene to see if
the values of control columns is different than the values in treated
columns.
...
Let’s put these two mean values into a new data.frame for easy
book-keeping and plotting.
...
If we consider “treated/control’ counts we will get a numner that tells
us the change
...
> Q. Add a new column `log2fc` for fold change of treated/control to our
> `meancounts` object.
...
full analysis for us on out `dds` input object:
...
> Q. Use ggplot2 to make a first version “Volcano plot” of
...
ggplot(res)+
  ...
  geom_vline(xintercept = c(-2,+2), col="grey", lty=2)+ 
  geom_hline(yintercept = -log(0.05), col="grey", lty=2)
```
...
We will use 2 bioconductor packages for this “mapping” **AnnotationDbi**
amd **org.Hs.eg.db**.
...
find out pathway they are involved in. This is called “pathway analysis”
or “gene set enrichment”
...
wants two main inputs: a vector of importance (in or case the log2 fold
...
```
    ...
    Info: Working in directory C:/Users/black/Documents/BIMM 143/bimm143_github/class13
